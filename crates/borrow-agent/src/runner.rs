//! Foreground runs started over SSH by `borrow run`.
//!
//! The command runs in its own process group so Borrow can stop all of it. When SSH
...
use borrow_core::control::{JobKind, JobState};
...
pub async fn run(
    ...
    let directory = match &project {
        Some(id) => {
            ...
            if !directory.is_dir() {
                bail!(
                    "{} does not exist in the Agent copy. Run borrow sync if it was just created",
