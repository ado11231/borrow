...
//!
//! SSH authenticates every caller. A hidden `borrow internal-control` helper, started
...
use borrow_core::{storage, telemetry};
use std::fs::File;
...
/// Keeps the daemon lock for as long as `borrow serve` runs.
pub struct Service {
    _guard: File,
}
...
pub fn start(name: String) -> anyhow::Result<Service> {
    ...
    let guard = storage::try_lock(&root.join("daemon.lock"))?
        .context("Another borrow serve is already running for this account")?;
    let path = socket(&root);
    if path.as_os_str().len() >= 100 {
        bail!(
            "The Borrow storage path is too long for a control socket: {}. Use an account with a shorter home directory",
            path.display()
        );
    }
...
pub async fn bridge() -> anyhow::Result<i32> {
    ...
    let stream = UnixStream::connect(&path)
        .await
        .context("Borrow is not serving on the Agent. Start borrow serve there")?;
...
mod tests {
    ...
    use borrow_core::control::ProjectRef;
    ...
    fn serve(root: &Root) -> PathBuf {
        let path = PathBuf::from(format!("/tmp/borrow-{}.sock", &storage::new_id()[..12]));
    ...
    async fn a_mismatched_version_is_answered_with_an_update_message() {
        ...
        assert!(matches!(reply, Response::Error(message) if message.contains("Update Borrow")));
    }
