//! `borrow ps` and `borrow stop`: Borrow runs and sessions on the Agent.
...
pub fn render(agent: &str, jobs: &[Job], all: bool, now: u64, style: Style) -> String {
    let heading = match all {
        true => format!("Borrow jobs on {agent}"),
        false => format!("Active Borrow jobs on {agent}"),
    };
    let mut output = format!("{}\n", style.heading(heading));
    if jobs.is_empty() {
        output.push_str(match all {
            true => "  No jobs recorded\n",
            false => "  Nothing running. See finished jobs with borrow ps --all\n",
        });
        return output;
    }
...
mod tests {
    ...
    fn jobs_are_listed_with_short_ids_and_readable_ages() {
        ...
        assert!(text.contains("Borrow jobs on archbox"));
    ...
    fn an_empty_list_explains_itself() {
        ...
        assert!(text.contains("Nothing running. See finished jobs with borrow ps --all"));
    }
}
