...
pub fn require(path: Option<PathBuf>) -> anyhow::Result<Local> {
    ...
    locate(&start).with_context(|| {
        format!(
            "No project found at {}. Use a folder inside a Git repository or one with Cargo.toml, package.json, pyproject.toml, or borrow.toml",
            start.display()
        )
    })
}
...
mod tests {
    ...
    impl Temp {
        fn new() -> Temp {
            let path = std::env::temp_dir().join(format!("borrow-cli-{}", storage::new_id()));
