    ...
    pub fn explain(&self, name: &str) -> Option<String> {
        match self {
            ...
            Diagnosis::Offline => Some(format!(
                "{name} is not reachable. It may be off or asleep, or borrow serve is not running there"
            )),
            Diagnosis::NotPaired => Some(format!(
                "{name} no longer accepts this machine. Run borrow link again from the same network as {name}"
            )),
...
mod tests {
    ...
    fn agent_root() -> PathBuf {
        let root = std::env::temp_dir().join(format!(
            "borrow-diagnose-{}",
            borrow_core::storage::new_id()
        ));
        borrow_core::storage::private_dir(&root).unwrap();
        root
    }
    ...
    async fn real_agent(root: &std::path::Path) -> Endpoint {
        ...
        tokio::spawn(borrow_agent::tunnel::accept(
            agent.clone(),
            root.to_path_buf(),
            sshd,
        ));
    ...
    async fn a_paired_client_is_diagnosed_as_reachable() {
        ...
        borrow_agent::clients::allow(&root, "laptop", &client.id().to_string()).unwrap();
